...
function createBracket(errorCallback) {
    var userId = window.location.hash;
    if (userId.substr(0, 2) !== '#!') {
        errorCallback('Invalid URL provided.');
        return;
    }
    userId = userId.replace(/(^#!)/, '');
    if (userId === '') {
        errorCallback('No player ID provided.');
        return;
    }
    ...
    q.await(function(error, inputBracket, keyData, standingsData) {
        var userData = standingsData.filter(function(d) {
            return d.userid === userId;
        });

        if (userData.length === 0) {
            errorCallback('Invalid player ID provided.');
            return;
        }
        userData = userData[0];

        var userName = userData.name;
        var bracketCode = userData.bracket_code;
        ...
        playerNameElement.text(userName);
        applyBracketCode(bracketCode);
